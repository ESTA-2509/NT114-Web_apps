...
index=0
index_name=0
sql="SELECT COUNT(*) as count_cate FROM (SELECT COUNT(`App_Id`), `Categories` FROM `google_play_apps_data_sample_1` GROUP BY `Categories` ORDER BY COUNT(`App_Id`)) count_cate"
...
for k in range(0, count_cate, 1):
        sql="SELECT COUNT(`App_Id`) FROM `google_play_apps_data_sample_1` GROUP BY `Categories` ORDER BY COUNT(`App_Id`) DESC LIMIT " + str(index) + ", 1"
        mycursor.execute(sql)
...
for m in range(0, count_cate, 1):
        sql="SELECT `Categories` FROM `google_play_apps_data_sample_1` GROUP BY `Categories` ORDER BY COUNT(`App_Id`) DESC LIMIT " + str(index_name) + ", 1"
        mycursor.execute(sql)
